...
Matrix256 *getGlobalAttentionMatrix(UnifiedInputProcessor *processor) {
    if (!processor) return nullptr;

    return processor->getGlobalAttentionMatrix();
}


// 检查处理器是否还有更多数据块

bool hasMoreBlocks(UnifiedInputProcessor *processor) {
    if (!processor) return false;

    return processor->hasMoreBlocks();
}


// 获取处理器的总块数

int getTotalBlocks(UnifiedInputProcessor *processor) {
    if (!processor) return 0;

    return processor->getTotalBlocks();
}


// 获取处理器的内容类型名称

const char *getContentTypeName(UnifiedInputProcessor *processor) {
    if (!processor) return "unknown";

    return processor->getContentTypeName();
}

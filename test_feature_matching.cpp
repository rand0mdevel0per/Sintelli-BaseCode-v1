...
void testBasicFeatureMatching() {
    ...
    std::cout << "存储的数据槽ID: " << slot1 << ", " << slot2 << ", " << slot3 << std::endl;
    ...
    std::cout << "相似度搜索结果:" << std::endl;
    for (const auto& item : similar_items) {
        TestData data;
        if (storage.fetch(item.first, data)) {
            std::cout << "槽ID: " << item.first 
                      << ", 相似度: " << item.second 
                      << ", 数据: " << data.name << std::endl;
        }
    }
...
void testFeatureHashLookup() {
    ...
    std::cout << "特征Hash: " << feature_hash << std::endl;
    std::cout << "找到的槽ID: ";
    for (auto id : slots) {
        std::cout << id << " ";
    }
